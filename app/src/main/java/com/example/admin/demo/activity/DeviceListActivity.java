 ...
 * /**
 *
 * @author VaiBhavsaR.uddin, RONGTA
...
public class DeviceListActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (pairedDevices.size() > 0) {
            ...
            for (BluetoothDevice device : pairedDevices) {
                String BD = device.getName();
                String BD1 = device.getAddress();
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN_MR2) {
                    String BD2 = String.valueOf(device.getType());
                }
                if(BD.contains("BlueTooth Printer")) {
    ...
    private void permissionChecking() {
        ...
        if (CURRENT_ANDROID_VERSION >= Build.VERSION_CODES.LOLLIPOP) { //Marshmallow
    ...
    private OnItemClickListener mDeviceClickListener = new OnItemClickListener() {
        public void onItemClick(AdapterView<?> av, View v, int arg2, long arg3) {
            ...
            String address1 = info.substring(info.length() - 35);

            // Create the result Intent and include the MAC address
            if (address1.contains("BlueTooth Printer")) {
                Intent intent = new Intent();
                intent.putExtra(EXTRA_DEVICE_ADDRESS, address1);
