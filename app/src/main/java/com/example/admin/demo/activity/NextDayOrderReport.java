...
public class NextDayOrderReport extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemSelectedListener {
    ...
    private String strTripId,strRouteId,strTripName;
    private String routeId,tripId,tripName,routeName,strCompanyName;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_next_day_order_report);
        ...
        tv_app_name_nav_bar.setText("Next Day Order Report");
        Functions.setToolBar(NextDayOrderReport.this, toolbar_vehicle_report,
                "Add Item", true);
        ...
        protected void onPostExecute(String result) {
            ...
            uiHandler1.post(new Runnable() {
                ...
                public void run() {
                    ...
                    ArrayAdapter<String> dataAdapter = new ArrayAdapter<String>(NextDayOrderReport.this,
    ...
    private class AsyncCallVehicleReport extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            vehicleReportAdapter = new VehicleReportAdapter(getVehicleReport(partyId,
                    routeId,tripId,invDate),NextDayOrderReport.this );
            Handler uiHandler = new Handler(Looper.getMainLooper());
            uiHandler.post(new Runnable() {
                @Override
                public void run() {
                    recyclerView.setHasFixedSize(true);
                    recyclerView.setLayoutManager(new LinearLayoutManager(NextDayOrderReport.this));
        ...
        protected void onPostExecute(String result) {
            ...
            if (result.equals("success")) {
                Toast.makeText(NextDayOrderReport.this, "Success", Toast.LENGTH_SHORT).show();
            } else if (result.equals("error")) {
                Toast.makeText(NextDayOrderReport.this, "Error", Toast.LENGTH_SHORT).show();
    ...
    public ArrayList<VehicleReportPojo> getVehicleReport(String catId, String routeId,
        ...
        String SOAP_ACTION = "http://tempuri.org/NextDayOrderReports";
        String METHOD_NAME = "NextDayOrderReports";
        String NAMESPACE = "http://tempuri.org/";
        ...
        try {
            ...
            if (!TextUtils.isEmpty(responseJSON)) {
                ...
                Toast.makeText(NextDayOrderReport.this, "Error From Server", Toast.LENGTH_SHORT).show();
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            ...
            public void run() {
                try {
                    ...
                    if (mPairedDevices.size() > 0) {
                        ...
                        BILL = BILL + String.format("%1$-23s %2$9s %3$9s", "Item", "Qty", "Unit");
                        BILL = BILL + "\n";
                        BILL = BILL
                                + "-----------------------------------------------";
                        ...
                        for (int i=0;i<vehicleReportPojoArrayList.size();i++){
                            ...
                            BILL = BILL + "\n " + String.format("%1$-23s %2$9s %3$9s", name, df.format(qty), unit);
                        }
                        ...
                        Toast.makeText(NextDayOrderReport.this, "No device Connected", Toast.LENGTH_SHORT).show();
                    }
